...
ds <- read.csv2("rifiuti_porto_genova_2008_2017_eda_0.csv", header=TRUE, sep = ";", check.names = FALSE, row.names=1)
...
**Dataset dimension:** `r dim(ds) ` (observations of variables)

**Exploratory Data analysis:**
\
Dataset contains **`r vuoti ` missing records** (NA).

Visually, is reported below a heatmap showing evidence of the dataset status (see annexes for bigger image).
...
p <- ggplot(ds,aes(x=year,y=CER,fill=value))+
        ...
        scale_fill_gradient(low = "grey96", high = "darkred", na.value = "grey96")
        theme(
...
ggsave("heatmap.png", scale = 1, limitsize = TRUE)
```
